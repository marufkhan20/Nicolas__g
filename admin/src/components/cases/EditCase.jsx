...
const EditCase = () => {
  ...
  // get case
  const { data: caseData } = useGetCaseQuery(id);

  useEffect(() => {
    if (caseData?._id) {
      setThumbnail(caseData?.thumbnail);
      setTitle(caseData?.title);
      setCategory(caseData?.category);
      setServiceProvided(caseData?.serviceProvided);
      setVideoUrl(caseData?.videoUrl);
      setProjectLogo(caseData?.projectLogo);
      setDescription(caseData?.description);
      setProjectUrl(caseData?.projectUrl);
      setCredits(caseData?.credits);
  ...
  // create new case
  const [updateCase, { data: updatedCase, isLoading, isError, error }] =
    useUpdateCaseMutation();
  ...
  useEffect(() => {
    ...
    if (!isLoading && updatedCase?._id) {
      toast.success("Case Updated Successfully");
      router("/cases");
    }
  }, [updatedCase, isLoading, isError, error, router]);
